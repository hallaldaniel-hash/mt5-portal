...
def create_mt5_account(
    *,
    ...
    """Create a trackable MT5 account inside a group.

    The account usually starts as pending. It becomes live only after the admin
    confirms it should be included in group balance/profit tracking.
    """
    ...
    return MT5Account(
        account_id=account_id or login.strip(),
        ...
        login=_require_text(login, "MT5 login"),
        sync_password=SecretValue(sync_password),
        investor_login=_require_text(investor_login, "Investor login"),
...
def client_visible_mt5_account(
    ...
    This follows visibility option C: account nickname, balances, and read-only
    login info. Master/sync passwords are intentionally excluded.
    """
    ...
    visible = {
        ...
        "read_only_notice": "Read-only access. Cannot trade, withdraw, or modify account.",
    }
